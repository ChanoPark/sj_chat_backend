...
class UserManager(BaseUserManager): #User를 생성할때 쓰는 헬퍼클래스, 장고모델들은 Manager를 통해 쿼리셋을 받음
    def create_user(self, user_id, username, nickname,classnum, email, password=None, **extra_fields): # **extra_fields *이 2개면 나머지 값을 딕셔너리로 바꿔서 저장하고, 1개는 tuple로 저장한다.
    ...
    def create_superuser(self, user_id, username, nickname, classnum, email, password=None, **extra_fields):
...
class User(AbstractBaseUser, PermissionsMixin):  #실제 모델이 상속받아 생성하는 클래스
    ...
    REQUIRED_FIELDS = ['username','nickname', 'email', 'classnum' ]
